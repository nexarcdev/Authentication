...
internal sealed class OpenIdConnectTokenExchangeOptionsConfigurator(
    string providerKey,
    string? scheme,
    ...
    ILoggerFactory loggerFactory) : IConfigureNamedOptions<OpenIdConnectOptions>
{
    ...
    {
        var configuredScheme = scheme ?? ResolveScheme();
        if (!string.Equals(name, configuredScheme, StringComparison.Ordinal))
            return;
    ...
    {
        var descriptor = providers.LastOrDefault(provider =>
            string.Equals(provider.ProviderKey, providerKey, StringComparison.OrdinalIgnoreCase));

        return descriptor?.Scheme ?? providerKey;
    }
}

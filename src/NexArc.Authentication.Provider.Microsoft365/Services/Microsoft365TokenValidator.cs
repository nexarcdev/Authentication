...
public sealed class Microsoft365TokenValidator : IExternalIdentityValidator
{
    ...
    private readonly ILogger<Microsoft365TokenValidator> _logger;

    public Microsoft365TokenValidator(
        IOptions<Microsoft365Options> options,
        ILogger<Microsoft365TokenValidator>? logger = null)
    {
        _options = options.Value;
        _logger = logger ?? NullLogger<Microsoft365TokenValidator>.Instance;
    ...
    public async Task<ExternalIdentity> ValidateAsync(ExternalTokenContext tokenContext, CancellationToken ct)
    {
        ...
        try
        {
            ...
            EnforceTenantAllowList(principal, tokenClaims.TenantId);
        ...
        catch (SecurityTokenException ex)
        {
            ...
            _logger.LogWarning(
                ex,
                "Microsoft365 token validation failed. ProviderKey={ProviderKey}, ConfigAuthority={Authority}, ConfigClientId={ClientId}, ConfigAllowedTenants=[{AllowedTenants}], Issuer={Issuer}, Audience={Audience}, TenantId={TenantId}",
    ...
    private void EnforceTenantAllowList(ClaimsPrincipal principal, string? tokenTenantId)
    {
        ...
        var tenantId = NormalizeTenantId(
            GetClaimValue(
                principal,
                "tid",
        ...
        {
            tenantId = NormalizeTenantId(tokenTenantId);
        }
